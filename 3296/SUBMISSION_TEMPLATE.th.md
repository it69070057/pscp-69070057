...
```text
โจทย์ให้รับ ค่าสีrgb 2สี แต่ละสีประกอบด้วย red(r) green(g) blue(b) แต่ละค่าอยู่ในช่วง 0 - 255 จากนั้นนำค่าสีแต่ละช่องสีที่อยุ่ตำแหน่งเดียวกันมาหาค่าเฉลี่ย เพื่อคำนวณหาค่าสีใหม่ที่เกิดจากการผสมกัน แสดงค่าRGB ของสีที่ผสมได้ ออกมาเป็นเลข 3ตัว คั่นแต่ละค่าด้วยช่องว่าง
```
...
Step 2:
เอาค่าสีที่1 และสีที่2 ในช่องเดียวกันมาหาค่าเฉลี่ย
r = (int(color1[0]) + int(color2[0])) // 2
g = (int(color1[1]) + int(color2[1])) // 2
b = (int(color1[2]) + int(color2[2])) // 2

color1[0] = ค่าR ของสีที่1
color2[0] = ค่าR ของสีที่2

int() = เปลี่ยนค่าข้อความเป็นตัวเลข
เอามาบวกกัน // 2 เพื่อหาค่าเฉลี่ย r = เก็บค่า R ที่ผสมแล้ว
...
```text
ทดสอบการหาค่าค่าเฉลี่ยของ RGB ทั่วไป
```
...
```text
ทดสอบค่าต่ำสุดที่เป็นไปได้ของRGB
```
