...
```text
input = เป็นจำนวนจริงจึงใช้float
หาคะแนนของคนที่ได้คะแนนต่ำสุดจาก total - ( 2 * max ) ถ้าคะแนนสูงสุด - ต่ำสุด ห่างกันมากกว่า2ตามเงื่อนไขโจทย์ ก็จะออกมาเป็นsurprising
```
...
```text
input = เป็นจำนวนจริงจึงใช้floatทั้งการรับคาคะแนนรวมและคะแนนสูงสุด
โดยการหาคะแนนต่ำสุด  จากการที่มีคนโหวต3คนถ้ามีคนให้คะแนนเต็ม2คน คะแนนของคนที่ได้คะแนนต่ำสุดจะเหลือจาก total - ( 2 * max )
โดยที่ถ้าคำนวณคะแนนออกมาติดลบจะปัดเป็น0ทันที
ต่อไปคือเช็คช่องว่างระหว่างคะแนน ถ้าคะแนนสูงสุด - ต่ำสุดออกมาได้มากกว่า2ปริ้น Surprisingนอกเหนือจากเื่อนไขนั้ยปริ้นNot surprising
```
...
```text
เช็คคำตอบว่าเมื่อเข้าสูตรหาค่าต่ำสุดแล้วถูกลบด้วยค่าสูงสุดจะออกมาตรงตามเงื่อนไขของโจทย์มั้ย โดยที่คำตอบต้องออกมาว่า Not surprising เนื่องจาก เมื่อหาminจะได้ 8 เมื่อลบmax จะได้2ซึ่งยังไม่ห่างกันเกิน2คะแนน
```
...
```text
เช็คคำตอบว่าเมื่อเข้าสูตรหาค่าต่ำสุดแล้วถูกลบด้วยค่าสูงสุดจะออกมาตรงตามเงื่อนไขของโจทย์มั้ย โดยที่คำตอบต้องออกมาว่า Surprising เนื่องจาก เมื่อหาminจะได้ -15 ระบบจะปัดเป็น0 เมื่อลบmax จะได้ผลต่าง36 ทันที 
```
...
```text
เช็คคำตอบว่าเมื่อเข้าสูตรหาค่าต่ำสุดแล้วถูกลบด้วยค่าสูงสุดจะออกมาตรงตามเงื่อนไขของโจทย์มั้ย โดยที่คำตอบต้องออกมาว่า Surprising เนื่องจาก เมื่อหาminจะได้ -31 ระบบจะปัดเป็น0 เมื่อลบmax จะได้ผลต่าง 40 ทันที 
```
