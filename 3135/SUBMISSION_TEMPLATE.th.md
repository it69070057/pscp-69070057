...
```text
รับ input 3ตัว
คือ จำนวนคนทั้งหมด จำนวนตำแหน่งที่ส่งต่อไป
หมายเลขของขโมย

เริ่มนับคนที่1 นับว่าเขาตรวจสอบได้แล้ว
ส่งของขวัญไปข้างหน้า k คน ถ้าเกินคนที่ n ให้วนกลับไปคนที่ 1
คนที่ได้ของขวัญจะได้รับการตรวจสอบ
หยุดตอนที่ ของขวัญกลับมาที่คนที่ 1
หรือ ของขวัญไปถึงขโมย
output แสดงจำนวนคนทที่ ได้ตรวจสอบของขวัญจริงๆ
...
```text
Step 1: กำหนดให้ของขวัญเริ่มที่คนี่ 1 คนี่ 1 เป็ฯคนแรกที่ได้พิจรณาของขวัญ จากยั้ยเริ่มให้นับจำนวนคนที่ตรวจสอบเป็น 1

Step 2: ส่งของขวัญที่ละ k ตำแหน่ง จากคนปัจจุบันไป k ตำแหน่ง เช่น ถ้า  k = 4 จะขยับ 4 ตำแหน่ง หากเลขกินคน หรือ n ให้กลับไปนับต่อจากคนที่ 1 เพาะคนนนั่งเป็นวงกลม จากนั้นนับจำนวนคนที่ได้ของขวัญเพิ่มขึ้น
Step 3: เช็กว่าต้องหยุดมั้ย หลังจากของขขวัญไปที่คนใหม่ ให้เช็กว่าคนนั้นเป็นขโมยรึป่าว  ถ้าเป็นขโมยใหหยุดทันที ถ้าของขวัญวนกลับมาที่คนี่ 1 ให้หยุดเช่นกัน แล้วแสดงจำนวนคนทั้งหมดที่ได้พิจรณาของขวัญจริงๆ
```
...
```text
ทดสอบกรณีส่งของขวัญทีละ 1 คน จะหยุดเมื่อถึงขโมย
```
...
```text
เช็ค 1 1 ไป 4 กลับไป 1 แล้วจะวนกลบมาที่คนที่ 1 กอ่นถึงขโมย คนที่ตรวจสอบคือ 1 กับ 4
```
...
```text
เช็ก 1 ไป 4 ไป 7 เมื่อถึงคนทที่ 7 ซึ่งเป็นขโมยก็จะหยุดทันที แปลว่ามีคนให้ตรวจสอบ 3 คน
```
