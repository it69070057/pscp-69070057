...
```text
หาจำนวนเฉพาะ input มี 2 ค่า จำนวนช่วงริ่มต้น กับ จำนวนสุดท้าย

ให้ output แสดง 
จำนวนเฉพาะที่อยู่ในช่วงนั้น จำนวนของจำนวนเฉพาะ
...
```text
Step 1: กำหนด input 2 ค่า ใช้ .split เพราะรับค่ามา 2 ตัว และ  map แปลงเป็น int โดยการใช้ map

Step 2: for n in range(start, end + 1):
        if n < 2:
            continue
    ...
    ใช้ for loop เพื่อไล่ตรวจเลขทุกตัวตั้งแต่ start to end ถ้า n < 2 จะข้ามเพราะจำนวนเฉพาะต้อง มากกว่าเท่ากับ 2
Step 3: 
if primes:
    print(*primes)
...
ปริ้นจำนวนเฉพาะใช้lenนับว่ามีกี่ตัว
```
...
```text
ทดสอบช่วงที่มีจำนวนเฉพาะหลายตัว และมีเลขที่ไม่ใช่จำนวนเฉพาะปนกันไป
```
...
```text
ทดสอบกรณีที่ตัวเริ่มและจบเป็นตัวเดียวกัน และเลขนั้นเป็นจำนวนเฉพาะ
```
...
```text
ทดสอบว่า โปรแกรมสามารถข้ามเลขที่น้อยกว่า2 แล้วหาจำนวนเฉพาะต่อได้ไหม
```
