...
```text
คำนวณยอดหลังหักส่วนลด ของนักเรียนดูจากสถานะสมาชิกและคำสั่งซื้อรวม

input
บรรทัดแรก สถานะสมาชิค
บรรทักสอง จำนานของที่ซื้อ
บรรทัดสาม ราคาสินค้าแต่ละรายการ n บรรทัด

output
แสดงราคาหลังหักส่วนลดเป็นทศนิยม 2 ตำแหน่ง

เงื่อนไขส่วนลดคือ y ลด 50%
n กับยอดรวม >= 500 จะลด 3%
นอกเหนือจากนี้คือไม่ลด
...
```text
Step 1: รับข้อมูลและรวมราคาสินค้า
รับสถานะสมาชิกและจำนวนสินค้า จากนั้นใช้ for รับราคาสินค้าแต่ละรายการแล้วรวมไว้ใน total

Step 2: คำนวณส่วนลด
ตรวจสอบสถานะสมาชิกด้วย if
ถ้าเป็น Y ลด 5% โดยคูณ 0.95
ถ้าเป็น N และยอดรวมตั้งแต่ 500 บาทขึ้นไป ลด 3% โดยคูณ 0.97
กรณีอื่นไม่ลดฃtotal = 0
    for _ in range(n):
        price = float(input())
        total += price
    ...
    FINAL_PRICE = int(total * 100 + 0.50001)/100
Step 3: ปัดเศษและแสดงผล
FINAL_PRICE ใช้ :.2f เพื่อทำเป็นทศนิยม 2 ตำแหน่ง
```
...
```text
ทดสอบกรณีสมาชิก Y ว่าลด 5% ถูกต้องมั้ย เพราะ 
ยอดรวม = 300 → 300 × 0.95 = 285
```
...
```text
ทดลองเงื่อนไข N กับยอด >= 500
ยอดรวม = 550 → 550 * 0.97 = 533.50
```
...
```text
ลองกรณีที่ไม่เข้าเงื่อนไขส่วนลดใดๆ เพราะเป็น N และยอดรวม 400 
```
