...
```text
โจทย์ต้องการให้ รับไพ่ 1 ใบ เช่น QS จากนั้นแปลงเป็นชื่อเต็ม
เลยใช้ input เป็น card แล้วใช้  .lower แปลงเป็นพิมพ์เล็ก output ต้องออกมาเป็น ชื่อแต้มและชื่อดอกไผ่มาต่อกัน
...
rank = card[:-1] (ตัวก่อนสุดท้าย)
suit = card[-1] (ตัวสุดท้าย)
...
ใช้ dictionary แปลงรหัสเป็นจำนวนเต็ม เช่น  q = queen , s = spades 
ถ้าเป็น 10c  rank    คือ 10 ซึ่งไม่มีในranks จึงใช้ 10 เหมือนเดิม

Step 3: 
print(f"{rank} of {suit}")
นำชื่อแต้มกับชื่อดอกไพ่มาต่อกัน
```
...
```text
ทดสอบว่า ถ้าแต้มเป็นตัวเลข + ตัวพิมพ์เล็ก
```
