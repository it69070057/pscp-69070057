...
```text
input = text = input().lower เพราะ รับทั้งตัวเล็กตัวใหญ่รวมกัรเลยให้แปลงให้เป็นตัวเล็กให้หมด

กำหนด ตัวสระ vowels = ['a' , 'e' , 'i' , 'o' , 'u']
count = [0,0,0,0,0] นับจำนวนสระเพิ่มโดยเริ่มจาก 0

ใช้ for loop นับ จำนวนตัวสสระ ในinput
ปริ้นจำนวนตัวสระแต่ละตัวที่มีออกมา
...
```text
Step 1: รับtext = input().lower()
Step 2:กำหนด การตัวสระvowels = ['a' , 'e' , 'i' , 'o' , 'u']
และเริ่มการนับ จำนวนโดย เริ่มที่ 0 count = [0,0,0,0,0] เอาเข้าลูปเพื่อนับจำนวน สระ ถ้าเจอ ก็ให้ countเพิ่มทีะ 1
for ch in text:
    if ch == 'a':
        count[0] += 1
    elif ch == 'e':
        ...
        count[4] += 1
Step 3:ใช้คำสั่งลูปอีกรอบ วนลูปตั้งแต่ 0 - 4 เพื่อตรวจ count แต่ละช่อง
if count[i] > 0 ถ้าสระมีมากกว่า 0 ตัวจะ๔ุก print ออกมา
f"{vowel[i] : count[i]}" คือการดึงตัวอักษรที่คู่กับจำนวนที่นับได้ออกมาแสดง
```
...
```text
ทดลองนับข้อความที่เป็นสระทั้งหมด แต่มีทั้งพิมพ์เล็กและพิมพ์ใหญ่
```
...
```text
ทดลองนับ ข้อความที่มีทั้ง พิมพ์เล็กและพิมพ์ใหญ่
```
