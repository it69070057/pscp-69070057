...
```text
input = s,n แทนพื้นที่น้ำหมึก และ คำร้องจากผู้คน

ใ้ชwhile loppให้โค้ดทำงานซ้ำจนครบnรอบ รับค่าพิกัดจุดx,y ใช้สูตรหาพื้นที่ printคำตอบออกมาโดยใช้math.ceilเพื่อปัดเศษเป็นจำนวนเต็มเสมอ และ  i += 1 เพิ่มค่าตัวนับลูปให้เช็๕เงื่อนไขถัดไป
```
...
```text
Step 1:รับinput โดยใช้input().split()เราจะรับข้อมูลมา2อันและแยกเป็นคนละค่า แต่ จะได้ออกมาเป็นสตริงจึวตต้องใช้map(int)แปลงจากสตริงเป็นเลข ทั้งs,n และ x,y
Step 2:ใส่สูตรคำนวณหาพื้นที่
Step 3:ปริ้นoutputออกมา
```
...
```text
เช็กกรณีจุดเริ่มต้น (0,0) เพราะว่าต้องได้ 0
```
...
```text
เช็คว่าถ้าใส่เดือน 9 คำตอบจะออกมาเป็นฤดูร้อนอย่างที่ควรมั้ย
```
