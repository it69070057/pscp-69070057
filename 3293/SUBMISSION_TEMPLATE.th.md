...
```text
โจทย์ให้รับข้อความ 5 บรรทัดมา แต่ละบรรทัดมีตัวอักษรกับตัวเลขหรือว่าเป็นช่องว่างก็ได้ แล้วก็ให้เอาข้อความ5บรรทัด มาทำเป็นกรอบ4เหลี่ยม โดยที่ต้องมีช่องว่าง 1 ช่องระหว่างข้อความกับกรอบทั้ง2ด้าน ด้่านซ้ายกับขวาต้องห่างกัรอย่างน้อย 2 ช่อง แล้วกรอบทั้งหมดต้องมีความกว้างเท่ากันทุกบรรทัด เลยต้องดูความยาวของข้อความที่ยาวที่สุดเพื่อกำหนดขนาดกรอบ
```
...
```text
สร้างlist สำหรับเก็บข้อความ 5บรรทัด

รับข้อความมา 5 บรรทัด เก็บไว้นlist 

หาความยาวของข้อความที่ยาวที่สุด

กำหนดความกว้างของกรอบจากข้อความที่ยาวที่สุด

กำหนดความกว้างของกรอบจากข้อความที่ยาวที่สุด ต้องมีช่องว่างฝั่ง้ายขวา อย่างละ 1 ช่อง

แสดงกรอบด้านบนออกมา

ใช้loop วนข้อความ 5บรรทัด
for _ in range(5):
    text.append(input())
...
for line in text:
    print("* " + line + " " * (max_lenght - len(line)) + " *")
...
```text
ทดสอบว่ารับข้อมูลแสดงตัวเลขแบบที่เป็น string ออกมาถูกต้องไหม
```
...
```text
ทดสอบตัวอักษรทั้งพิมพ์เล็ก พิมพ์ใหญ่ โดยที่ไม่เปลี่ยนตัวอักษร
```
