...
```text
input รับค่าต้นทางถึงปลายทางโดยใช้.strip เพื่อลบช่องว่า
หาก ผู้พิมเผลอใส่ช่องว่างมาจะไม่เขาเงื่อนไขและผลจะออกerror ทันที และ น้ำหนัก โดยแยกบรรทัด น้ำหนัก เป็นทศนิยมได้เลยรับค่าเป็น float จากนั้นสร้างเงื่อนไขคำนวณราคาค่าทำเนียมรวมกับน้ำหนัก
...
```text
Step 1: รับinput 2ค่า นั่นคือ
route = input().strip()
weight = float(input())

Step 2: สร้างเงื่อนไชเพื่อให้คำนวณ ว่าไปรอบไหน ต้องคิดค่าธรรมเนียมและน้ำหนักรวมกันเท่าไหร่
if route == "BKK CNX":
    total_price = 10 + (weight * 30)
    print(f"{total_price:.2f}")
elif route == "CNX UBP":
    total_price = 15 + (weight * 40)
    print(f"{total_price:.2f}")
elif route == "UBP BKK":
    total_price = 20 + (weight * 40)
    print(f"{total_price:.2f}")
elif route == "BKK PKT":
    total_price = 25 + (weight * 50)
    print(f"{total_price:.2f}")
elif route == "PKT CNX":
    total_price = 30 + (weight * 60)
    print(f"{total_price:.2f}")
elif route == "UBP PKT":
    total_price = 40 + (weight * 70)
    print(f"{total_price:.2f}")
Step 3: ถ้าข้อมูลที่รับมาไม่มีตรงกับรอบที่กำหนด จะออกมาเป็น error
else:
    print("Error")
```
...
```text
ทดลองใช้เงื่อนไขตามโจทย์ว่า คำตอบที่ออกมาถูกต้องไหม โดยใส่ช่องว่างเข้าไปด้วย
```
...
```text
ทดลองใช้เงื่อนไขตามโจทย์ว่า คำตอบที่ออกมาถูกต้องไหม
```
...
```text
ทดลองใช้เงื่อนไขตามโจทย์ว่า คำตอบที่ออกมาถูกต้องไหม แต่่ลงอใส่เป็นพิมพ์เล็ก ซึ่งคำตอบควรออกมาเป็น error เพราะรับแค่ตัวพิมพ์ใหญ่เท่านั้น
```
