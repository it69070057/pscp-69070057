...
```text
input: (ตามที่โจทย์กำหนดให้ 4 บรรทัด)
a = int(input())
b = int(input())
d = int(input())
r = int(input())

count_num = 0 เก็บผลรวมการนับเริ่มจาก0
for i ing range(a , b+1)
วนลูปตรวจสอบ หยุดที่ b+1 ไม่งั้น b จะไม่ถูกเช็ค
ถ้า i % d = r คือ ถ้า i หารด้วย d แล้วได้เศษ r
ก็จะได้นับตัวเลขเพิ่มอีก1ตัว นั่นคือ count_num += 1
แล้วprint(count_num) ออกมาว่านับได้กี่ตัว
...
```text
Step 1:รับinput เป็นint 4 ตัวตามที่โจทย์กำหนด
Step 2:ใช้ for loop เพื่อวนลูปนับจำนวนตั้งแต่(a,b)เลขที่หาร d ได้เศษ r ออกมา
Step 3:ปริ้นcount_num ออกมา
```
...
```text
เช็คว่าตั้งแต้ (10 , 50) ว่าตัวที่หาร5แล้วเหลือเศษ 2 ได้ 8 ตัวไหม นั่น คือ 12,17,22,27,32,37,42,47
```
...
```text
ทดสอบว่าคำตอบออกมาเป็น 0ไหม เพราะ ถ้า หาร 3 เศษที่ออกมาจะเป็น 3 ไม่ได้
```
...
```text
เช็คว่าตั้งแต้ (1 , 10) ว่าตัวที่หาร2แล้วเหลือเศษ 1 ได้  ตัวไหม นั่น คือ 1,3,5,7,9
```
